...
let AnalyticsService = class AnalyticsService {
    ...
    getMaturityLevel(riskFactors) {
        ...
        if (highPercent > 60)
            return { level: 'M1', ...this.maturityLevels['M1'] };
        if (lowPercent > 80)
            return { level: 'M5', ...this.maturityLevels['M5'] };
        if (highPercent + moderatePercent >= 40 && highPercent + moderatePercent <= 60)
            return { level: 'M2', ...this.maturityLevels['M2'] };
        if (moderatePercent >= 30 && moderatePercent <= 40)
            return { level: 'M3', ...this.maturityLevels['M3'] };
        if (moderatePercent >= 10 && moderatePercent < 30)
            return { level: 'M4', ...this.maturityLevels['M4'] };
        if (highPercent + moderatePercent > 30)
            return { level: 'M2', ...this.maturityLevels['M2'] };
        return { level: 'M4', ...this.maturityLevels['M4'] };
    }
    calculateDataForResponses(responses) {
        ...
        const riskFactors = allDimensionIds.map((id) => {
        ...
        const formattedDistributions = Object.fromEntries(Object.entries(distributions).map(([dimId, dist]) => {
    ...
    async getDashboardData(companyId, filters) {
        ...
        const userFilter = { role: 'COLLABORATOR' };
        if (filters.unidade)
            userFilter.branchId = filters.unidade;
        if (filters.genero)
            userFilter.genero = filters.genero;
        if (filters.nivelCargo)
            userFilter.nivelCargo = filters.nivelCargo;
        if (filters.area)
            userFilter.area = filters.area;
