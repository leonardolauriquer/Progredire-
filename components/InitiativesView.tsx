...
const InitiativeCard: React.FC<{
    ...
    return (
        <div className="bg-[--color-card] p-6 rounded-2xl shadow-lg border border-[--color-border] space-y-4">
            <div>
                <div className="flex justify-between items-start gap-2">
                    <h3 className="text-xl font-bold text-[--color-card-foreground]">{initiative.factor}</h3>
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${getStatusStyles(initiative.status)}`}>
                        {initiative.status}
                    </span>
                </div>
                <p className="text-sm text-[--color-card-muted-foreground]">Público: {initiative.segment}</p>
            </div>

            <blockquote className="bg-[--color-muted] border-l-4 border-blue-500 p-4 rounded-r-lg">
                <p className="text-[--color-card-muted-foreground] italic">"{initiative.announcement}"</p>
            ...
            <div className="space-y-3">
                <div>
                    <h4 className="font-semibold text-[--color-card-foreground] flex items-center gap-2"><FlagIcon className="w-5 h-5 text-green-600"/> Objetivo</h4>
                    <p className="text-sm text-[--color-card-muted-foreground] pl-7">{initiative.objective}</p>
                </div>
                <div>
                    <h4 className="font-semibold text-[--color-card-foreground] flex items-center gap-2"><PaperAirplaneIcon className="w-5 h-5 text-indigo-600"/> Principais Ações</h4>
                    <ul className="list-disc list-inside pl-7 space-y-1 text-sm text-[--color-card-muted-foreground] mt-2">
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <div className="text-center">
                ...
                <h1 className="text-3xl font-bold text-[--color-foreground]">Mural de Iniciativas</h1>
            ...
            {initiatives.length > 0 ? (
                <div className="space-y-6">
                 ...
                 <div className="text-center py-16 bg-[--color-card] rounded-xl shadow-lg border border-[--color-border]">
                    ...
                    <h2 className="text-xl font-semibold text-[--color-card-foreground]">Nenhuma iniciativa publicada ainda.</h2>
                    <p className="text-[--color-card-muted-foreground] mt-2">Quando a empresa compartilhar um plano de ação, ele aparecerá aqui.</p>
                </div>
            )}
        </div>
    );
